...
export const fetchSuccess = createAction(
  ...
  ({ items, totalCount }: { items: Opportunity[]; totalCount: number }): Partial<IOpportunityStateContext> => ({
    isPending: false,
    isSuccess: true,
    isError: false,
...
export const setSelectedAction = createAction(
  ...
  (selected: Opportunity | null): Partial<IOpportunityStateContext> => ({ selected })
);